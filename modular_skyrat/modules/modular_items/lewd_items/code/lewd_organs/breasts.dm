...
// Increased all sizes originals litsed on the right
/obj/item/organ/genital/breasts/build_from_dna(datum/dna/DNA, associated_key)
	...
	switch(genital_type)
		if("pair")
			breasts_capacity = 10 //2
		if("quad")
			breasts_capacity = 20 //2.5
		if("sextuple")
			breasts_capacity = 30 //3
	...
	// this is started over in fluid_generation.dm
