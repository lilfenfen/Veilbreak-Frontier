...
/datum/preferences/proc/save_custom_tattoo_data()
	if(!parent?.mob) return
	var/mob/living/carbon/human/H = parent.mob
	if(!istype(H) || QDELETED(H)) return
	...
	// Write to a dedicated preference entry (ensure the entry exists)
...
/datum/preferences/proc/load_custom_tattoo_data()
	...
	for(var/i in 1 to length(tattoo_data))
		...
		if(!body_part_string) continue
		var/body_part_define = get_custom_tattoo_standardized_body_part(body_part_string)
		if(!body_part_define) continue
		if(!is_custom_tattoo_bodypart_valid(body_part_define)) continue
		...
		var/datum/custom_tattoo/T = new(final_artist, final_design, body_part_define, final_color, final_layer, final_is_signature, final_font)
		if(date_applied) T.date_applied = sanitize_text(date_applied)
		loaded_tattoos += T
	features["custom_tattoos"] = loaded_tattoos

/datum/preferences/proc/apply_custom_tattoos_to_mob(mob/living/carbon/human/H)
	if(!istype(H) || !features["custom_tattoos"]) return
	H.custom_body_tattoos.Cut()
	var/list/saved_tattoos = features["custom_tattoos"]
	if(!islist(saved_tattoos)) return
