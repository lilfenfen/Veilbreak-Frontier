...
/mob/living/carbon/human/proc/add_tattoo(datum/tattoo/new_tattoo)
	...
	var/current_tattoos = length(get_tattoos(new_tattoo.body_part))
	if(current_tattoos >= MAX_TATTOOS_PER_PART)
		return FALSE

	LAZYADD(body_tattoos, new_tattoo)
...
/mob/living/carbon/human/Initialize(mapload)
	. = ..()
	addtimer(CALLBACK(src, .proc/load_tattoos_from_prefs), 1 SECONDS)

/mob/living/carbon/human/proc/load_tattoos_from_prefs()
	if(client?.prefs)
		client.prefs.apply_tattoos_to_mob(src)
