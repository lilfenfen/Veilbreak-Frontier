...
/datum/portal_destination/veilbreak/proc/initialize_dungeon_subsystems(z_level)
	...
	// NEW: Ensure all walls have proper base_icon_state and smoothing
	ensure_wall_smoothing_compatibility(z_level)
	CHECK_TICK
...
/// Ensure all walls have proper base_icon_state and smoothing compatibility
/datum/portal_destination/veilbreak/proc/ensure_wall_smoothing_compatibility(z_level)
	log_dungeon("Wall Compatibility: Ensuring all walls can smooth properly on Z-level [z_level]")

	var/fixed_count = 0

	// Ensure all walls have proper base_icon_state
	for(var/turf/closed/wall/wall in world)
		if(wall.z == z_level)
			// Ensure base_icon_state is set to "wall" for consistent smoothing
			if(!wall.base_icon_state || wall.base_icon_state == "")
	...
	log_dungeon("Wall Compatibility: Fixed [fixed_count] walls with base_icon_state")
