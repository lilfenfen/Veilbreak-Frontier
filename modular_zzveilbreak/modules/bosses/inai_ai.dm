...
/datum/ai_controller/inai/process(seconds_per_tick)
	...
	if(astral_action && !astral_action.on_cooldown && target && get_dist(inai, target) <= 11)
		astral_action.Activate(target)
		return

	// Try to use Resonant Wave if ready
	var/datum/action/cooldown/mob_cooldown/resonant_wave/wave_action = locate() in inai.actions
	if(wave_action && !wave_action.on_cooldown)
