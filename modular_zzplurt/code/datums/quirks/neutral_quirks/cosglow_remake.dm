...
/datum/action/cosglow/update_glow/Trigger(trigger_flags)
	...
	*/
	...
	// The filter must be removed before being re-added with new properties.
	action_mob.remove_filter("rad_fiend_glow")
	// Update outline effect. The opacity needs to be converted to a two-digit hex string.
	var/hex_opacity = num2hex(glow_opacity, 2)
	action_mob.add_filter("rad_fiend_glow", 1, outline_filter("color" = "[glow_color][hex_opacity]", "size" = glow_thickness))
